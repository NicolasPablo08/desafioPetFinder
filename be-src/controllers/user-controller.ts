...
export async function createUser(email: string, password: string) {
  if (!email || !password) throw new Error("falta el email o la contraseña para crear el perfil");
  try {
...
export async function loginUser(email: string, password: string) {
  if (!email || !password) throw new Error("falta el email o la contraseña para buscar el perfil");
  try {
    const user = await User.findOne({
      where: { email },
    });
...
export async function setNameAndLocalidad(name: string, localidad: string, id: number) {
  if (!name || !localidad || !id)
    throw new Error("falta el nombre la localidad o el id para cargar en el perfil");
  try {
...
export async function setNewPassword(password: string, id: number) {
  if (!password || !id) throw new Error("falta el nuevo password o el id para cargar en el perfil");
  try {
    const passwordHash = await getHash(password);
...
export async function createReport(
  ...
  if (!name || !lat || !lng || !imageUrl || !userId)
    throw new Error("Falta el nombre, locacion, imagen o userId de la mascota a reportar");
  try {
    ...
    if (!uploadImageToCloudinary) throw new Error("No se pudo subir la imagen a cloudinary");
...
export async function editReport(
  ...
  if (!petId || !userId) throw new Error("Falta el petId o el userId para editar el reporte");
  try {
...
export async function getPetsLostUser(userId: number) {
  if (!userId) throw new Error("Falta el userId para obtener las mascotas perdidas");
  try {
...
export async function getOnePetLostUser(userId: number, reportId: number) {
  if (!userId || !reportId)
    throw new Error("Falta el userId o el reportId para obtener la mascota perdida");
  try {
...
export async function deleteReport(userId: number, petId: number) {
  if (!userId || !petId) throw new Error("Falta el userId o el reportId para eliminar el reporte");
  try {
