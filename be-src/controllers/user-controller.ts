...
export async function createUser(email: string, password: string) {
	if (!email || !password) {
		console.error(
			"falta el email o el password en la funcion createUser del userController"
		);
		return { status: "error" };
	}
	try {
...
export async function loginUser(email: string, password: string) {
	if (!email || !password) {
		console.error(
			"falta el email o el password en la funcion loginUser del userController"
		);
		return { status: "error" };
	}
	try {
...
export async function setNameAndLocalidad(
	...
	try {
		...
		console.error(
			"Error en la funcion setNameAndLocalidad del userController",
			error
		);
		return { status: "error" };
	}
...
export async function setNewPassword(password: string, id: number) {
	...
	try {
		...
		console.error(
			"Error en la funcion setNewPassword del userController",
			error
		);
		return { status: "error" };
	}
...
export async function createCode(email: string) {
	...
	try {
		...
		const writeCodeInDB = await User.update(
			{ code, timeExpireCode },
			{ where: { email } }
		);
		if (writeCodeInDB[0] === 0) {
			console.error(
				"No se pudo guardar el codigo en la db en la funcion createCode"
			);
			return { status: "error" };
		}
...
export async function compareCode(codigo: string, email: string) {
	if (!codigo || !email) {
		console.error(
			"falta el codigo o el email en la funcion compareCode del userController"
		);
		return { status: "error" };
	}
	try {
		...
		if (code === codigo && diferencia > 0) {
			...
			return {
				status: "success",
				message: "El codigo ingresado es correcto",
				token,
			};
		} else if (diferencia <= 0) {
			return {
				status: "warning",
				message: "El codigo ya expiro, solicita uno nuevo",
			};
		} else {
			return {
				status: "warning",
