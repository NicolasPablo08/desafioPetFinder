...
export async function createReport(
	...
	if (!name || !lat || !lng || !imageUrl || !userId) {
		console.error(
			"falta el nombre, latitud, longitud, imagen o id del usuario en la funcion createReport del petController"
		);
		return { status: "error" };
	}
	try {
		...
		if (!uploadImageToCloudinary) {
			console.error(
				"No se pudo subir la imagen a cloudinary en la funcion createReport del petController"
			);
			return { status: "error" };
		}
		...
		if (!newPetReport) {
			console.error(
				"No se pudo crear el reporte en la funcion createReport del petController"
			);
			return { status: "error" };
		}
		...
		if (!algoliaRes) {
			console.error(
				"No se pudo guardar el reporte en algolia en la funcion createReport del petController"
			);
			return { status: "error" };
		}
		return {
		...
		console.error("Error en la funcion createReport del petController", error);
		return { status: "error" };
	}
}
...
export async function getPetsLostUser(userId: number) {
	if (!userId) {
		console.error(
			"falta el id del usuario en la funcion getPetsLostUser del petController"
		);
		return { status: "error" };
	}
	try {
		...
		if (!petsLost) {
			console.error(
				"No se pudieron obtener las mascotas perdidas en getPetsLostUser del petController"
			);
			return { status: "error" };
		}
		...
		return {
			status: "success",
			message: "Mascotas perdidas obtenidas con exito",
			petsLost,
		};
	} catch (error) {
		console.error(
			"Error en la funcion getPetsLostUser del userController",
			error
		);
		return { status: "error" };
	}
...
export async function editReport(
	...
	if (!petId || !userId) {
		console.error(
			"falta el id de la mascota o el id del usuario en la funcion editReport del petController"
		);
		return { status: "error" };
	}
	try {
		...
		if (imageUrl) {
			...
			if (!uploadImageToCloudinary) {
				console.error(
					"No se pudo subir la imagen a cloudinary en la funcion editReport del petController"
				);
				return { status: "error" };
			}
		...
		if (!editReport[0]) {
			console.error(
				"No se pudo editar el reporte en la funcion editReport del petController"
			);
			return { status: "error" };
		}
		...
		if (!algoliaRes) {
			console.error(
				"No se pudo actualizar el reporte en algolia en la funcion editReport del petController"
			);
			return { status: "error" };
		}
		...
		if (!reportPet) {
			console.error(
				"No se pudo obtener el reporte editado en la funcion editReport del petController"
			);
			return { status: "error" };
		}
		return {
			status: "success",
			message:
				"Se actualizo la publicacion de tu mascota, suerte con la busqueda!",
			reportPet,
		};
	} catch (error) {
		console.error("Error en la funcion editReport del petController", error);
		return { status: "error" };
	}
}
...
export async function deleteReport(userId: number, petId: number) {
	if (!userId || !petId) {
		console.error(
			"falta el id del usuario o el id de la mascota en la funcion deleteReport del petController"
		);
		return { status: "error" };
	}
	try {
		...
		if (reportToDelete) {
			...
			return {
				status: "success",
				message: " Se elimino la publicación de tu mascota",
			};
		} else {
			return {
				status: "warning",
		...
		console.error("Error en la funcion deleteReport del petController", error);
		return { status: "error" };
	}
}
...
export async function getOnePetLostUser(userId: number, reportId: number) {
	if (!userId || !reportId) {
		console.error(
			"falta el id del usuario o id de la mascota en la funcion getOnePetLostUser del petController"
		);
		return { status: "error" };
	}
	try {
		...
		console.error(
			"Error en la funcion getOnePetLostUser del petController",
			error
		);
		return { status: "error" };
	}
...
export async function sendEmail(
	...
	if (!email || !nombre || !telefono || !informacion || !namePet) {
		console.error(
			"falta el email, el nombre, el telefono , la informacion o el nombre de la mascota en la funcion sendEmail del petController"
		);
		return { status: "error" };
	}
	try {
