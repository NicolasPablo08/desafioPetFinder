...
app.patch("/me/mis-datos", authMiddleware, async (req, res) => {
  ...
  try {
    const user = await setNameAndLocalidad(req.body.name, req.body.location, req._user.id);
...
app.patch("/me/mi-pass", authMiddleware, async (req, res) => {
  ...
  if (!req._user) return res.status(401).json({ message: "Token no autorizado" });
  try {
    const newToken = await setNewPassword(req.body.password, req._user.id);
    return res.json(newToken);
  } catch (error) {
    return res.status(204).json({ message: "Error al cambiar el password", error });
  }
});
...
app.post("/me/my-pets", authMiddleware, async (req, res) => {
  if (!req.body.name || !req.body.lat || !req.body.lng || !req.body.imageUrl)
    throw new Error("Falta el nombre, locacion o imagen de la mascota a reportar");
  if (!req._user) return res.status(401).json({ message: "Token no autorizado" });
  try {
...
app.patch("/me/my-pets/:petId", authMiddleware, async (req, res) => {
  ...
  if (!req.params.petId) return res.status(400).json({ message: "Falta el id del reporte" });
...
app.get("/me/my-pets", authMiddleware, async (req, res) => {
  ...
  try {
    ...
    return res.status(204).json({ message: "Error al obtener las mascotas perdidas", error });
  }
});
...
app.delete("/me/my-pets/:petId", authMiddleware, async (req, res) => {
  ...
  if (!req.params.petId) return res.status(400).json({ message: "Falta el id del reporte" });
  try {
    const deleteOneReport = await deleteReport(req._user.id, Number(req.params.petId));
...
app.post("/send-email", async (req, res) => {
  ...
  if (!email || !nombre || !telefono || !informacion || !namePet)
    return res.status(400).json({
      message: "Falta el email, nombre, telefono, informacion o el nombre de la mascota",
    });
  try {
    const response = await SendEmail(nombre, email, telefono, informacion, namePet);
