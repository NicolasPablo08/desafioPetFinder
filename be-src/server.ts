...
app.patch("/me/my-data", authMiddleware, async (req, res) => {
	...
	try {
		const result = await setNameAndLocalidad(
			req.body.name,
			req.body.location,
			req._user.id
		);
...
app.patch("/me/my-pets/:petId", authMiddleware, async (req, res) => {
	...
	if (!req.params.petId) {
		console.error(
			"Falta el id de la mascota en /me/my-pets/:petId PATCH de la API"
		);
		return { status: "error" };
	}
	...
	if (!name || !imageUrl || !lat || !lng) {
		console.error(
			"Faltan datos para editar el reporte en /me/my-pets/:petId PATCH de la API"
		);
		return {
			status: "warning",
...
app.delete("/me/my-pets/:petId", authMiddleware, async (req, res) => {
	...
	if (!req.params.petId) {
		console.error(
			"Falta el id de la mascota en /me/my-pets/:petId DELETE de la API"
		);
		return { status: "error" };
	}
	try {
