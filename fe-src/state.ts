...
export const state = {
  ...
  async logIn(email: string, password: string) {
    ...
    try {
      ...
      cb.user.email = email;
      this.setState(cb);
      const petsCharge = await this.getAllPetsUser(); //cargamos el state con todos los pets del user
      return petsCharge;
    } catch (error) {
  ...
  async createPetReport(name: string, imageUrl: string, lat: number, lng: number) {
    ...
    try {
      ...
      if (!respuesta.ok) throw new Error("Error al crear el reporte de la mascota");
  ...
  async searchPetsNearby(lat: number, lng: number, rango: number) {
    ...
    try {
      ...
      console.error("Error en al realizar las busqueda con el metodo searchPetsNearby:", error);
    }
  },
  //enviar email
  async sendEmail(
