...
export const state = {
  ...
  async createPetReport(name: string, imageUrl: string, lat: number, lng: number) {
    const cb = this.getState();
    const token = cb.user.token;
    try {
      const response = await fetch(LOCAL_URL + "/me/my-pets", {
      ...
      if (data.status === "success") {
        ...
        const ubicacion = await getCiudadProvincia(data.newPetReport.lat, data.newPetReport.lng);
        const pet = {
  ...
  async editPetReport(name: string, imageUrl: string, lat: number, lng: number, petId: number) {
    const cb = this.getState();
    const token = cb.user.token;
    try {
      ...
      const data = await response.json(); //asi devuelve la respuesta el metodo http
      if (data.status === "success") {
        const ubicacion = await getCiudadProvincia(data.reportPet.lat, data.reportPet.lng);
  ...
  async searchPetsNearby(lat: number, lng: number, rango: number) {
    ...
    try {
      const response = await fetch(
        LOCAL_URL + "/search-pets?lat=" + lat + "&lng=" + lng + "&rango=" + rango
      );
      const data = await response.json();
      if (data.status === "success") {
        const searchPets = await Promise.all(
