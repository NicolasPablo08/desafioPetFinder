  ...
  class PetLostPage extends HTMLElement {
    ...
    render() {
      ...
      formButton.addEventListener("click", async (e) => {
        ...
        const informacion = formInformacion.shadowRoot.querySelector("textarea").value;
        form.style.filter = "blur(5px)";
        loadComp.style.display = "inherit";
        if (!nombre || !telefono || !informacion) {
          form.style.filter = "none";
          loadComp.style.display = "none";
          messageComp.style.display = "inherit";
          messageComp.textContent =
            "Completa todos los campos del formulario para enviar el mensaje!";
          setTimeout(() => {
            messageComp.style.display = "none";
          }, 4000);
          return;
        }
        const response = await state.sendEmail(nombre, userEmail, telefono, informacion, namePet);
        if (response.status === "success" || response.status === "warning") {
          ...
          loadComp.style.display = "none";
          messageComp.style.display = "inherit";
          messageComp.textContent = " Error al enviar el email, intenta mas tarde!";
          setTimeout(() => {
            Router.go("/");
          }, 4000);
        }
        ...
        formInformacion.shadowRoot.querySelector("textarea").value = "";
