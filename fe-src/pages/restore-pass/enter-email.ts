	...
	class EnterEmailPage extends HTMLElement {
      ...
      <div class="data">
        ...
        <div class="buttons">
          ...
          <button-comp class="button-back" variant="gray">Volver</button-comp>
        </div>  
      </div>  
      ...
      `;
			...
			buttonForm.addEventListener("click", async (e) => {
				...
				if (response.status === "success") {
					...
					messageComp.textContent =
						"Hubo un error, intenta nuevamente mas tarde";
					setTimeout(() => {
						Router.go("/login");
					}, 4000);
				}
			...
			async function getCode(email: string) {
				try {
					...
					console.error(
						"error en la funcion getCode de la page enter-email",
						error
					);
					return { status: "error" };
				}
