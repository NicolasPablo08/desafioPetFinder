...
export function enterCodePage() {
  class EnterCodePage extends HTMLElement {
    ...
    render() {
      ...
      <button-comp class="button-back" variant="blue">Volver a enviar el código</button-comp>
      </div>
      </div>  
      ...
      buttonForm.addEventListener("click", async (e) => {
        ...
        if (response.status === "success") {
          ...
          dataContainer.style.filter = "none";
          loadComp.style.display = "none";
          messageComp.textContent = "Error desconocido, intente nuevamente mas tarde";
          messageComp.style.display = "inherit";
          setTimeout(() => {
            state.logOut();
          }, 4000);
      ...
      async function sendCode(codigo: string) {
        try {
          ...
          console.error("error en la funcion sendCode de la page enter-code", error);
          return { status: "error" };
        }
      }
      function timer() {
