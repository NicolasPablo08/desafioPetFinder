...
export function initMap(divMap: Element) {
  ...
  }).setView([-34.6, -58.38], 13); //coordenadas para que inicie en la ciudad de buenos aires
  ...
  map.on("click", (e) => {
    ...
    // Creamos uno nuevo
    currentMarker = L.marker([lat, lng], { icon: customIcon }).addTo(map).openPopup();
